...
class SQL_Constructor:
     def  __init__(self):
          ...
          ```
          4. Điểm chuẩn thpt ngành khoa học máy tính UIT và HCMUS 2022
          ```
          SELECT *
          FROM admission_scores
          WHERE year = 2022
...
if __name__ == "__main__":         
     ...
     sql = SQL_Constructor()
     query = "Điểm chuẩn thpt ngành khoa học máy tính UIT và HCMUS 2022"
     sql_query = sql.transform_to_sql(query)
     formatted = sql.format_query(sql_query)
     df = sql.run(formatted)
     print(sql_query)
     print(formatted)
     print(df)